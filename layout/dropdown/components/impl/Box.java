    ...
    public void render(int mouseX, int mouseY) {
        ...
        block5: {
            block4: {
                ...
                cropX = this.x + 5;
                cropY = Math.min((float)this.parent.y + (float)((double)(this.parent.height - ClickGUI.defaultHeight)) + (float)ClickGUI.defaultHeight, (float)(this.height + this.y));
                cropWidth = this.width - 10;
                cropHeight = Math.min(this.y + this.height - (this.parent.y + 12), this.height);
                if (cropHeight <= 0.0f) break block4;
                float f = this.height;
                float f2 = this.lastBox ? 4.5f : 1.0f;
                if (!(f <= f2)) break block5;
            }
            return;
        }
        GuiRenderUtils.endCrop();
        GuiRenderUtils.beginCrop(cropX, cropY, cropWidth, cropHeight, 2.0f);
        this.children.forEach(c -> c.render(mouseX, mouseY));
        GuiRenderUtils.endCrop();
        GuiRenderUtils.beginCrop(this.parent.crX, this.parent.crY, this.parent.crXX, this.parent.crYY, 2.0f);
    }
