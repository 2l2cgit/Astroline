...
extends Component {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        float y = this.y + 2;
        float margin = 2.0f;
        this.list_rect = new Rect((float)this.x + margin, y, (float)this.width - margin * 2.0f, this.height - 18);
        GuiRenderUtils.beginCrop((float)this.x + margin, y + (float)(this.height - 18) - 0.5f, (float)this.width - margin * 2.0f, (float)(this.height - 18) - 1.0f, 2.0f);
        for (int i = 0; i < PresetManager.presets.size(); ++i) {
            String preset = PresetManager.presets.get(i);
            if (i == this.index) {
                GuiRenderUtils.drawRect(x + 0.5f, y + 0.5f + this.scrollValue, (float)this.width - margin * 2.0f - 1.0f, FontManager.tiny.getHeight(preset) + 2.0f, Hud.hudColor1.getColorInt());
            }
            ...
            y += FontManager.tiny.getHeight(preset) + 2.0f;
        }
        GuiRenderUtils.endCrop();
        y = this.y + this.height - 14;
        GuiRenderUtils.drawBorderedRect(x, y, 24.0f, 12.0f, 0.5f, Hud.isLightMode.getValue().booleanValue() ? (this.isHoveredLoad ? -10044417 : -10049281) : (this.isHoveredLoad ? GuiRenderUtils.darker(-14145496, -30) : -14145496), Hud.isLightMode.getValue() != false ? new Color(ColorUtils.BLUE.c).getRGB() : Hud.hudColor1.getColorInt());
        FontManager.tiny.drawString("Load", x + 4.0f, y + 2.0f, new Color(0xF8F8F8).getRGB());
        this.load_rect = new Rect(x, y, 24.0f, 12.0f);
        x = this.x + 76;
        GuiRenderUtils.drawBorderedRect(x, y, 12.0f, 12.0f, 0.5f, Hud.isLightMode.getValue().booleanValue() ? (this.isHoveredSave ? -10044417 : -10049281) : (this.isHoveredSave ? GuiRenderUtils.darker(-14145496, -30) : -14145496), Hud.isLightMode.getValue() != false ? new Color(ColorUtils.BLUE.c).getRGB() : Hud.hudColor1.getColorInt());
        FontManager.tiny.drawString("+", x + 4.0f, y + 2.0f, new Color(0xF8F8F8).getRGB());
        this.save_rect = new Rect(x, y, 12.0f, 12.0f);
        x = this.x + 62;
        if (this.canberemoved()) {
            GuiRenderUtils.drawBorderedRect(x, y, 12.0f, 12.0f, 0.5f, Hud.isLightMode.getValue().booleanValue() ? (this.isHoveredDelete ? -10044417 : -10049281) : (this.isHoveredDelete ? GuiRenderUtils.darker(-14145496, -30) : -14145496), Hud.isLightMode.getValue() != false ? new Color(ColorUtils.BLUE.c).getRGB() : Hud.hudColor1.getColorInt());
    ...
    public void mouseUpdates(int mouseX, int mouseY, boolean isPressed) {
        ...
        boolean bl = this.isHoveredDelete = MathUtils.contains(mouseX, mouseY, this.del_rect) && this.isHovered && this.canberemoved();
    ...
    public void mouseWheelUpdate(int mouseX, int mouseY, int amount) {
        if (amount == 0 || !MathUtils.contains(mouseX, mouseY, this.list_rect)) {
            return;
        }
    ...
    private void pressed(int mouseY) {
        ...
        if (this.isHoveredLoad) {
            ...
            preset = PresetManager.presets.get(this.index);
            PresetManager.loadPreset(preset);
            Astroline.INSTANCE.tellPlayer("Loaded preset " + (Object)((Object)EnumChatFormatting.YELLOW) + preset);
        }
        if (this.isHoveredSave) {
            Minecraft.getMinecraft().displayGuiScreen(new SavePresetScreen(Astroline.INSTANCE.getDropdown()));
        }
        if (this.isHoveredDelete && this.canberemoved()) {
            if (PresetManager.presets.size() - 1 < this.index) {
                return;
            }
            preset = PresetManager.presets.get(this.index);
            Minecraft.getMinecraft().displayGuiScreen(new GuiYesNo((result, id) -> {
                ...
                Minecraft.getMinecraft().displayGuiScreen(Astroline.INSTANCE.getDropdown());
            }, "Are you sure you want to remove the preset? " + (Object)((Object)EnumChatFormatting.YELLOW) + preset, "", 0));
        }
    }
