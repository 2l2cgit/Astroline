...
public class ConfigSlot {
    ...
    public void render(float x, float y) {
        ...
        RenderUtil.drawRect(x + 88.0f - this.animation, y - 23.0f, x + 90.0f, y - 5.0f, this.parent.backgroundColor);
        this.parent.drawGradientSideways(x + 68.0f - this.animation, y - 23.0f, x + 88.0f - this.animation, y - 5.0f, RenderUtil.reAlpha(this.parent.backgroundColor, 0.0f), RenderUtil.reAlpha(this.parent.backgroundColor, 1.0f));
    ...
    public void onClicked(float mouseX, float mouseY, float mouseButton) {
        if (mouseButton == 0.0f && RenderUtil.isHovering(mouseX, mouseY, this.x + 15.0f, this.y - 23.0f, this.x + 90.0f, this.y - 5.0f)) {
            if (!this.selected) {
                ...
                Minecraft.getMinecraft().displayGuiScreen(new GuiYesNo((result, id) -> {
                    ...
                    Minecraft.getMinecraft().displayGuiScreen(this.parent.clickGUI);
                }, "Are you sure to remove the preset named " + (Object)((Object)EnumChatFormatting.YELLOW) + this.configName + (Object)((Object)EnumChatFormatting.RESET) + " ?", "", 0));
                this.selected = false;
            }
        }
        if (mouseButton == 1.0f) {
            this.selected = RenderUtil.isHovering(mouseX, mouseY, this.x + 15.0f, this.y - 23.0f, this.x + 90.0f, this.y - 5.0f) ? !this.selected : false;
        }
    }
}
