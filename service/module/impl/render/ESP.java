...
extends Module {
    ...
    public static BooleanValue healthBar = new BooleanValue("ESP", "Health Bar", true);
    public static BooleanValue tags = new BooleanValue("ESP", "Nametags", true);
    ...
    public void onRender2D(Event2D event) {
        for (EntityPlayer player : this.entityPosMap.keySet()) {
            ...
            if (healthBar.getValue().booleanValue()) {
                ...
                Gui.drawRect(x - 2.0f, y, x - 1.0f, y2, col);
                if (needScissor) {
                    ESP.endScissorBox();
                }
            }
            if (tags.getValue().booleanValue()) {
                ...
                float scale = Math.max(minScale, Math.min(1.0f, 1.0f - ESP.mc.thePlayer.getDistanceToEntity(player) / 100.0f));
                float yOff = Math.max(0.0f, Math.min(1.0f, ESP.mc.thePlayer.getDistanceToEntity(player) / 12.0f));
    ...
    public void onRender3D(Event3D event) {
        ...
        if (box.getValue().booleanValue() || healthBar.getValue().booleanValue() || tags.getValue().booleanValue()) {
            int scaleFactor = scaledResolution.getScaleFactor();
            float partialTicks = event.getPartialTicks();
            for (EntityPlayer player2 : ESP.mc.theWorld.playerEntities) {
                if (player2.getDistanceToEntity(ESP.mc.thePlayer) < 1.0f) continue;
                GL11.glPushMatrix();
