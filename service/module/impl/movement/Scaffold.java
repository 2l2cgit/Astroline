...
extends Module {
    public static ModeValue keep_y_mode = new ModeValue("Scaffold", "Keep Y Mode", "Always", "Speed Only");
    public static BooleanValue keep_y = new BooleanValue("Scaffold", "Keep Y", false);
    public static BooleanValue tower_move = new BooleanValue("Scaffold", "Tower Move", false);
    public static BooleanValue tower = new BooleanValue("Scaffold", "Tower", false);
    public static BooleanValue swapper = new BooleanValue("Scaffold", "Swapper", false);
    public static BooleanValue silent = new BooleanValue("Scaffold", "Silent", false);
    ...
    public double[] getExpandCoords(double y) {
        ...
        double zCalc = -999.0;
        double dist = 0.0;
        double d = expandDist = Scaffold.mc.thePlayer.getDistanceY(this.blockBelow.getY()) == 1.0 ? 0.0 : 0.0;
        while (!this.isAirBlock(underBlock)) {
            ...
            xCalc += ((double)forward * 0.45 * (double)MathHelper.cos((float)Math.toRadians(yaw + 90.0f)) + (double)strafe * 0.45 * (double)MathHelper.sin((float)Math.toRadians(yaw + 90.0f))) * dist;
            zCalc += ((double)forward * 0.45 * (double)MathHelper.sin((float)Math.toRadians(yaw + 90.0f)) - (double)strafe * 0.45 * (double)MathHelper.cos((float)Math.toRadians(yaw + 90.0f))) * dist;
    ...
    public void onPreUpdate(EventPreUpdate event) {
        ...
        } else if (this.getBlockCountHotBar() <= 0) {
        ...
        if (!this.keepY() || Scaffold.mc.thePlayer.onGround) {
            this.floorY = MathHelper.floor_double(Scaffold.mc.thePlayer.posY);
        }
        ...
        if (this.blockData != null) {
            EntityPig entity = new EntityPig(Scaffold.mc.theWorld);
            entity.posX = (double)this.blockData.position.getX() + 0.5;
            entity.posY = (double)this.blockData.position.getY() + 0.5;
            entity.posZ = (double)this.blockData.position.getZ() + 0.5;
        ...
        } else if (Scaffold.mc.thePlayer.getDistanceY(this.blockBelow.getY()) == 1.0) {
    ...
    private void towerMove() {
        if (MoveUtils.isOnGround(0.76) && !MoveUtils.isOnGround(0.75) && Scaffold.mc.thePlayer.motionY > 0.23 && Scaffold.mc.thePlayer.motionY < 0.25) {
        ...
        } else if (Scaffold.mc.thePlayer.posY >= (double)Math.round(Scaffold.mc.thePlayer.posY) - 1.0E-4 && Scaffold.mc.thePlayer.posY <= (double)Math.round(Scaffold.mc.thePlayer.posY) + 1.0E-4 && !Keyboard.isKeyDown((int)Scaffold.mc.gameSettings.keyBindSneak.getKeyCode())) {
            Scaffold.mc.thePlayer.motionY = 0.0;
        }
    }
    ...
    public void onRightClick(RightClickEvent event) {
        if (this.blockData != null && (!this.towering() || this.canPlace)) {
            this.placeBlock();
        }
    }
    ...
    public void onTower(EventSendPacket event) {
        ...
        if (silent.getValue().booleanValue() && event.getPacket() instanceof C09PacketHeldItemChange && ((C09PacketHeldItemChange)(packet = (C09PacketHeldItemChange)event.getPacket())).getSlotId() != this.neededSlot()) {
            event.setCancelled(true);
        }
        if (event.getPacket() instanceof C03PacketPlayer) {
            packet = (C03PacketPlayer)event.getPacket();
            if (this.towering()) {
                this.canPlace = !((C03PacketPlayer)packet).isMoving();
            }
        }
    }

    public float getYawBackward() {
        float yaw = MathHelper.wrapAngleTo180_float(Scaffold.mc.thePlayer.rotationYaw);
    ...
    private void placeBlock() {
        if (silent.getValue().booleanValue()) {
            mc.getNetHandler().getNetworkManager().sendPacketWithoutEvent(new C09PacketHeldItemChange(this.neededSlot()));
        ...
        BlockPos pos = this.blockData.position;
        EnumFacing facing = this.blockData.facing;
        Vec3 vector = MathHelper.getVec3(pos, facing);
        if (Scaffold.mc.playerController.onPlayerRightClick(Scaffold.mc.thePlayer, Scaffold.mc.theWorld, stack, pos, facing, vector)) {
            if (!silent.getValue().booleanValue()) {
    ...
    public int getBlockCount() {
        ...
        for (int i = 0; i < 45; ++i) {
            ...
            if (!(is.getItem() instanceof ItemBlock) || this.getBlacklistedBlocks().contains(((ItemBlock)item).getBlock())) continue;
    ...
    public void onRender2D(Event2D event) {
        ...
        if (stack != null && stack.getItem() instanceof ItemBlock && this.isValid(stack.getItem())) {
            ...
            RenderUtil.drawFastRoundedRect((int)(x - 13.0f), y + height, (int)(x + width + 9.0f), y + height + 17.5f, 4.0f, new Color(0, 0, 0, 120).getRGB());
            RenderUtil.drawStack(Minecraft.getMinecraft().fontRendererObj, false, stack, x - 11.0f, y + 11.0f);
        }
        GL11.glPopMatrix();
    }
    ...
    private int neededSlot() {
        for (int i = 0; i < 9; ++i) {
            Item item;
            if (Scaffold.mc.thePlayer.inventory.getStackInSlot(i) == null || Scaffold.mc.thePlayer.inventory.getStackInSlot((int)i).stackSize == 0 || !this.isValid(item = Scaffold.mc.thePlayer.inventory.getStackInSlot(i).getItem())) continue;
            return i;
        }
    ...
    public void onDisable() {
        ...
        if (this.canDisable) {
            if (silent.getValue().booleanValue()) {
                mc.getNetHandler().getNetworkManager().sendPacketWithoutEvent(new C09PacketHeldItemChange(Scaffold.mc.thePlayer.inventory.currentItem));
            } else {
                Scaffold.mc.thePlayer.inventory.currentItem = this.oldSlot;
            }
            if (!Scaffold.mc.thePlayer.movementInput.sneak) {
                mc.getNetHandler().getNetworkManager().sendPacketWithoutEvent(new C0BPacketEntityAction(Scaffold.mc.thePlayer, C0BPacketEntityAction.Action.STOP_SNEAKING));
            }
            this.canDisable = false;
        }
        super.onDisable();
