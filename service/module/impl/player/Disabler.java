...
extends Module {
    ...
    public void onUpdate(EventPreUpdate event) {
        if (Astroline.INSTANCE.moduleManager.getModule("Speed").isToggled()) {
            ...
            yawDiff = MathHelper.wrapAngleTo180_float(targetYaw - angle.getYaw().floatValue());
    ...
    public void doInvMove(EventSendPacket e) {
        ...
        if (!(Disabler.mc.thePlayer.ticksExisted % 5 != 0 || this.clickWindowPackets.isEmpty() || isDraggingItem || this.isCraftingItem)) {
            Astroline.INSTANCE.tellPlayer("Release Click Packets");
            Minecraft.getMinecraft().getNetHandler().getNetworkManager().sendPacketWithoutEvent(new C16PacketClientStatus(C16PacketClientStatus.EnumState.OPEN_INVENTORY_ACHIEVEMENT));
            for (C0EPacketClickWindow clickWindowPacket : this.clickWindowPackets) {
                Minecraft.getMinecraft().getNetHandler().getNetworkManager().sendPacketWithoutEvent(clickWindowPacket);
            }
            Minecraft.getMinecraft().getNetHandler().getNetworkManager().sendPacketWithoutEvent(new C0DPacketCloseWindow(0));
    ...
    public void onPacket(EventReceivePacket e) {
        ...
        if (e.getPacket() instanceof C03PacketPlayer.C06PacketPlayerPosLook) {
            ...
            e.setPacket(new C03PacketPlayer.C04PacketPlayerPosition(c06.x, c06.y, c06.z, c06.isOnGround()));
