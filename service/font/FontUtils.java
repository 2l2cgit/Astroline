...
public class FontUtils {
    ...
    public void drawLimitedStringWithAlpha(String text, float x, float y, int color, float alpha, float maxWidth) {
        ...
        for (char c : characters) {
            ...
            if (c != '\u00a7' && (index == 0 || index == characters.length - 1 || characters[index - 1] != '\u00a7')) {
                ...
                GL11.glPushMatrix();
                this.unicodeFont.drawString(Character.toString(c), (double)x, (double)y, RenderUtil.reAlpha(currentColor, alpha), false);
                GL11.glPopMatrix();
    ...
    public int drawStringWithShadow(String text, float x, float y, int color) {
        this.drawStringWithAlpha(StringUtils.stripControlCodes(text), x + 0.5f, y + 0.4f, 0, (float)(color >> 24 & 0xFF) / 255.0f);
        return this.drawString(text, x, y, color);
    }

    public void drawStringWithSuperShadow(String text, float x, float y, int color) {
        this.drawStringWithAlpha(StringUtils.stripControlCodes(text), x + 0.8f, y + 0.8f, 0, (float)(color >> 24 & 0xFF) / 255.0f);
        this.drawString(text, x, y, color);
    }
