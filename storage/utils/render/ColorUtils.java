    ...
    public static long renderGradientRect(int left, int top, int right, int bottom, double time, long difference, long delay, HUD.RainbowDirection rainbowDirection) {
        ...
        if (rainbowDirection == HUD.RainbowDirection.RIGHT) {
            for (i = 0; i < right - left; ++i) {
                Gui.drawRect(left + i, top, right, bottom, ColorUtils.interpolateColorsBackAndForth(Hud.arrayListSpeed.getValue().intValue(), i, Hud.hudColor1.getColor(), Hud.hudColor2.getColor(), false).getRGB());
            }
        }
        ...
        if (rainbowDirection == null) {
            for (i = 0; i < bottom - top; ++i) {
                Gui.drawRect(left, top + i, right, bottom, ColorUtils.interpolateColorsBackAndForth(Hud.arrayListSpeed.getValue().intValue(), i, Hud.hudColor1.getColor(), Hud.hudColor2.getColor(), false).getRGB());
            }
        }
        if (rainbowDirection == HUD.RainbowDirection.UP) {
            for (i = 0; i < bottom - top; ++i) {
                Gui.drawRect(left, top + i, right, bottom, ColorUtils.interpolateColorsBackAndForth(Hud.arrayListSpeed.getValue().intValue(), i, Hud.hudColor1.getColor(), Hud.hudColor2.getColor(), false).getRGB());
            }
        }
